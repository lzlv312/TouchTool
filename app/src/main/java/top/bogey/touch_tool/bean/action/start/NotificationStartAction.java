...
public class NotificationStartAction extends StartAction {
    private final transient Pin appsPin = new Pin(new PinApplications(PinSubType.MULTI_APP, MainApplication.getInstance().getString(R.string.common_package)), R.string.pin_app) {
        @Override
        public boolean linkAble() {
            return false;
        }
    };
    private final transient Pin matchPin = new Pin(new PinString("."), R.string.notification_start_action_match) {
        @Override
        public boolean linkAble() {
            return false;
        }
